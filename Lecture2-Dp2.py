...
from intro import playIntro, changeLine
...
&\text{You are given an array of positive integers A, where A[i] represents a leaf node in a binary tree and}\\
&\text{A corresponds to an in-order traversal of the leaf nodes. For each non-leaf node, its value consists }\\
...
\end{align*}""",color = WHITE).scale(0.45)
        ...
        cc0 = MathTex(r"6", color = BLUE).scale(0.75)
        cc0.move_to(l[0].get_center())
        cc1 = MathTex(r"5", color = BLUE).scale(0.75)
        cc1.move_to(l[1].get_center())
        cc2 = MathTex(r"4", color = BLUE).scale(0.75)
        cc2.move_to(l[2].get_center())
        name1 = Text("A =").scale(0.65)
        name1.align_to(leafs, LEFT)
        name1.shift(LEFT / 1.25)

        leafs.add(cc0, cc1, cc2, name1)
        leafs.move_to(ORIGIN + DOWN / 2 + LEFT / 2)
        ...
        l2 = MathTex("5", color = BLACK).scale(0.7)
        ...
        n1 = Circle(color = WHITE, radius = 0.35, stroke_width = 1.4)
        n1.set_fill(color = DARK_BLUE, opacity = 0.9)

        c2 = Circle(color = WHITE, radius = 0.35, stroke_width = 1.4)
        c2.set_fill(color = DARK_BLUE, opacity = 0.9)

        n3 = Circle(color = WHITE, radius = 0.35, stroke_width = 1.4)
        n3.set_fill(color = DARK_BLUE, opacity = 0.9)

        c4 = Circle(color = WHITE, radius = 0.35, stroke_width = 1.4)
        c4.set_fill(color = DARK_BLUE, opacity = 0.9)

        c5 = Circle(color = WHITE, radius = 0.35, stroke_width = 1.4)
        c5.set_fill(color = DARK_BLUE, opacity = 0.9)


        l1 = MathTex("6", color = WHITE).scale(0.7)
        l2 = MathTex("5", color = WHITE).scale(0.7)
        l3 = MathTex("4", color = WHITE).scale(0.7)
...
\end{align*}""").scale(0.5)
        ...
        nl1 = Circle(color = WHITE, radius = 0.5, stroke_width = 1.4)
        nl1.set_fill(color = WHITE, opacity = 1)
        nl2 = Circle(color = DARK_BLUE, radius = 0.47, stroke_width = 1.4)
        nl2.set_fill(color = DARK_BLUE, opacity = 1)
        nl1.move_to(l[1].get_center() + 2.5 * UP)

        left = VGroup(l[0], l[1], cc0, cc1)
        right = VGroup(l[2], cc2)

        e1 = Line(start = nl1.get_center(), end = left.get_center() + UP / 2, stroke_width = 1.2)
        e2 = Line(start = nl1.get_center(), end = right.get_center() + UP / 2, stroke_width = 1.2)

        e1 = always_redraw(
            lambda: Line(start = changeLine(nl1.get_x(), nl1.get_y(), left.get_x(), left.get_y()) + nl1.get_center(), 
                         end = changeLine(left.get_x(), left.get_y(), nl1.get_x(), nl1.get_y()) + left.get_center() + UP / 2, 
                         stroke_width = 1.2)
        )
        e2 = always_redraw(
            lambda: Line(start = changeLine(nl1.get_x(), nl1.get_y(), right.get_x(), right.get_y()) + nl1.get_center(), 
                         end = changeLine(right.get_x(), right.get_y(), nl1.get_x(), nl1.get_y()) + right.get_center() + UP / 2, 
                         stroke_width = 1.2)
        )
        nl2.move_to(nl1.get_center())

        p1 = VGroup(e1, e2, nl1, nl2)
        self.play(FadeIn(p1))
        self.play(FadeOut(name1))
        self.play(left.animate.shift(LEFT))
        self.play(right.animate.shift(RIGHT))
        self.wait(3)

        tempScore = MathTex(r"\text{score = 6 * 4}").scale(0.7)
        finalScore = MathTex(r"24").scale(0.7)
        tempScore.move_to(nl1.get_center() + 2.5 * LEFT)
        finalScore.move_to(nl1.get_center())
        leftNums = VGroup(l[0], l[1])

        self.play(leftNums.animate.set_fill(color = WHITE, opacity = 0.2))
        self.wait(2)
        self.play(leftNums.animate.set_fill(color = WHITE, opacity = 0))
        ...
        self.play(FadeIn(finalScore))
        p1.add(finalScore)

        tempGroup = VGroup(p1, left, right)
        self.play(tempGroup.animate.shift(UP))
        self.wait(2)

        # show how there are sub-problems
        braceLeftObj = Brace(left, direction = DOWN, sharpness = 1)
        braceRightObj = Brace(right, direction = DOWN, sharpness = 1)
        braceLeftText = Tex(r"\text{sub-tree with smallest internal node sum given leaf nodes A[0 ... 1]}").scale(0.4).next_to(braceLeftObj, direction = DOWN)
        braceRightText = Tex(r"\text{sub-tree with smallest internal node sum given leaf nodes A[2 ... 2]}").scale(0.4).next_to(braceRightObj, direction = DOWN)
        ...
        tempGroup = VGroup(braceLeftObj, braceRightObj)
        tempGroupText = Tex(r"""
\begin{align*}
&\text{if we know the optimal solutions to these sub-problems, then we can use}\\
&\text{those solutions to find the optimal solution to our larger sub-problem!}\\
\end{align*}""").scale(0.4)
        tempGroupText.move_to([braceRightObj.get_x() + ((braceRightObj.get_x() - braceLeftObj.get_x()) / 2), 0, 0] + 2.25 * DOWN + 3 * LEFT)
        tempGroup.add(tempGroupText)
        self.wait(3)
        self.play(FadeIn(tempGroup))
        self.wait(5)
        self.play(FadeOut(tempGroup))
        self.play(left.animate.shift(RIGHT))
        self.play(right.animate.shift(LEFT))
        self.play(FadeOut(finalScore))
        self.wait()
        left = VGroup(l[0], cc0)
        right = VGroup(l[1], l[2], cc1, cc2)

        ee1 = Line(start = nl1.get_center(), end = left.get_center() + UP / 2, stroke_width = 1.2)
        ee2 = Line(start = nl1.get_center(), end = right.get_center() + UP / 2, stroke_width = 1.2)
        ...
        temp1 = VGroup(e1 ,e2, nl1, nl2)
        temp2 = VGroup(ee1, ee2, nl1, nl2)
        self.play(ReplacementTransform(temp1, temp2))
        self.play(right.animate.shift(RIGHT))
        self.play(left.animate.shift(LEFT))

        tempScore = MathTex(r"\text{score = 6 * 5}").scale(0.7)
        finalScore = MathTex(r"30").scale(0.7)
        tempScore.move_to(nl1.get_center() + 2.5 * LEFT)
        finalScore.move_to(nl1.get_center())
        rightNums = VGroup(l[1], l[2])


        self.play(l[0].animate.set_fill(color = WHITE, opacity = 0.2))
        self.wait(2)
        self.play(l[0].animate.set_fill(color = WHITE, opacity = 0))
        self.play(FadeIn(tempScore[0][0:7]))
        self.wait(2)
        self.play(rightNums.animate.set_fill(color = WHITE, opacity = 0.2))
        self.wait(2)
        self.play(rightNums.animate.set_fill(color = WHITE, opacity = 0))
        self.play(FadeIn(tempScore[0][7:10]))
        self.wait(2)
        self.play(FadeOut(tempScore))
        self.play(FadeIn(finalScore))
        ...
        braceLeftObj = Brace(left, direction = DOWN, sharpness = 1)
        braceRightObj = Brace(right, direction = DOWN, sharpness = 1)
        braceLeftText = Tex(r"\text{sub-tree with smallest internal node sum given leaf nodes A[0 ... 0]}").scale(0.4).next_to(braceLeftObj, direction = DOWN)
        braceRightText = Tex(r"\text{sub-tree with smallest internal node sum given leaf nodes A[1 ... 2]}").scale(0.4).next_to(braceRightObj, direction = DOWN)
        ...
        prev = VGroup(ee1, ee2, nl1, nl2, left, right, finalScore)
        ...
        state3 = Tex(r"""
\begin{align*}
&\texttt{Let dp[i][j] represent the minimum sum of non-leaf nodes in}\\
&\texttt{a binary tree consisting of the leaf nodes A[i...j]}\\
\end{align*}""").scale(0.7)
        state3.move_to(ORIGIN + UP / 2)
        self.play(FadeIn(state3))
        self.wait(5)
        self.play(FadeOut(state3))
        self.wait()
        self.play(FadeIn(prev))
        self.wait()
        self.play(prev.animate.shift(UP))
        t = VGroup(cc0, cc1, cc2, finalScore)
        self.play(FadeOut(t))
        t = VGroup(l[0], l[1], l[2])
        self.play(t.animate.set_fill(color = WHITE, opacity = 0.2))

        # show the partions
        helperText = Tex(r"\texttt{0 $\leq$ k $<$ j}").scale(0.6)
        helperText.move_to(ORIGIN + 5.5 * LEFT)
        self.play(FadeIn(helperText))
        self.wait(3)

        recurrence = Tex(r"""\begin{align*}
&\texttt{dp[i][j] = min(dp[i][j], dp[i][k] + dp[k + 1][j] + max(A[i...k]) * max(A[k + 1...j]))}
\end{align*}""").scale(0.5)
        recurrence.move_to(ORIGIN + 2 * DOWN)
        self.play(FadeIn(recurrence[0][0:9]))
        self.wait(2)
        self.play(FadeIn(recurrence[0][9:13]))
        self.wait(2)
        self.play(FadeIn(recurrence[0][13:22]))

        # Show left sub-problem
        braceObj = Brace(left, sharpness = 1, direction = DOWN)
        braceText = Tex(r"""\begin{align*}
&\texttt{sub-tree with smallest internal node sum given leaf nodes A[i...k]}
\end{align*}""").next_to(braceObj, DOWN).scale(0.5)
        brace = VGroup(braceObj, braceText)
        self.play(FadeIn(brace))
        self.wait(4)
        self.play(FadeOut(brace))
        self.play(FadeIn(recurrence[0][22:30]))
        self.wait(2)
        braceObj = Brace(right, sharpness = 1, direction = DOWN)
        braceText = Tex(r"""\begin{align*}
&\texttt{sub-tree with smallest internal node sum given leaf nodes A[k + 1...j]}
\end{align*}""").next_to(braceObj, DOWN).scale(0.5)
        brace = VGroup(braceObj, braceText)
        self.play(FadeIn(brace))
        self.wait(4)
        self.play(FadeOut(brace))
        self.play(FadeIn(recurrence[0][30:41]))
        self.wait(2)
        self.play(FadeIn(recurrence[0][41:75]))
        self.wait(7)

        end = VGroup(helperText, recurrence, l[0], l[1], l[2], ee1, ee2, nl1, nl2)
        self.play(FadeOut(end))

        text = MathTex(r"&\texttt{What about our base cases?}").scale(0.8)
        self.wait()
        self.play(FadeIn(text))
        self.wait(4)
        self.play(FadeOut(text))

        s = Square(side_length = 1, color = WHITE, stroke_width = 1)
        c0 = Tex(r"\texttt{A[i]}", color = BLUE).scale(0.75)
        c0.move_to(s.get_center())

        tempGroup = VGroup(s, c0)
        tempGroup.move_to(ORIGIN + 2 * UP)
        self.play(FadeIn(tempGroup))

        helperText = Tex(r"\texttt{We need at least 2 leaf nodes to make an internal node!}").scale(0.6)
        helperText.move_to(ORIGIN)
        self.play(FadeIn(helperText))